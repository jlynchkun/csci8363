function [Initial_W,W,H1,H2] = DNMF_mm_image_ge_learn_assoc_corr(X1, X2, W0, H10, H20, A, B, L1, L2, r1, r2, r3, K, maxiter, speak, fid, opt)
%
...
for iter = 1 : maxiter
    ...
    HH1 = H1.*(W'*X1 + L1*H1*A + L2*H2*H2'*H1 + H2*B')./((W'*W+r2*ones(K))*H1+ H2*H2'*H1 + eps);     %%Update H1 H2 simultaneously
    [HH1,d1] = norm_cols(HH1',2);
    HH1 = HH1';
    H2 = H2.*(W'*X2 + L2*H1*H1'*H2 + H1*B)./((W'*W+r2*ones(K))*H2+ H1*H1'*H2 + eps); 
